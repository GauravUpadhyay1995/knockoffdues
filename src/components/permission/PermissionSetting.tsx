                            ...
                            <motion.div initial={{ scale: 0.98, y: 8, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.98, y: 8, opacity: 0 }} className="relative w-full max-w-2xl rounded-2xl p-6 bg-gradient-to-br from-white/4 to-white/6 border border-white/8 backdrop-blur-3xl shadow-2xl">
                                ...
                                <div className="mt-4 space-y-3">
                                    ...
                                    <div className="flex gap-3 justify-end mt-3">
                                        <button  disabled={isSubmitting} onClick={() => setIsAddModuleOpen(false)} className="px-3 py-2 rounded-lg bg-white/6 text-white">{isSubmitting ? "Please Wait Saving..." : "Cancel"}</button>
                                        {/* <button  disabled={isSubmitting} onClick={() => addModule({ saveNow: false })} className="px-3 py-2 rounded-lg bg-blue-600 text-white">Add (local)</button> */}
                                        <button  disabled={isSubmitting} onClick={() => addModule({ saveNow: true })} className="px-3 py-2 rounded-lg bg-emerald-500 text-white"> {isSubmitting ? "Please Wait Saving..." : "Add & Save"}</button>
                                    </div>
                                </div>
                            </motion.div>
                        </motion.div>
