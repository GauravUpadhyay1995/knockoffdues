...
export default function DepartmentListTable({ initialData }: Props) {
    ...
    const fetchDepartments = useCallback(
        async (abortController?: AbortController) => {
            ...
            try {
                ...
                const response = await fetch(`/api/v1/admin/departments/list?${params}`, {
                    credentials: 'include',
                ...
                const result: DepartmentsApiResponse = await response.json();
    ...
    useEffect(() => {
        ...
        fetchDepartments(abortController);
    ...
    const handleEditClick = (dept: Department) => {
        router.push(`departments/update/${dept._id}`);
    };

    const changeStatus = async (dept: Department) => {
        if (!dept._id) return;
        ...
        const promise = fetch(`/api/v1/admin/departments/update/${dept._id}`, {
        ...
        toast.promise(promise, {
            ...
            success: (res) => res?.success ? 'Department updated successfully!' : null,
            error: (err) => err.message || 'Department Update failed',
        });
    ...
    const handleDownloadExcel = () => {
        const data = departments.map((dept, idx) => ({
        ...
        XLSX.writeFile(wb, `departments_export_${timestamp}.xlsx`);
    };
    const handleNameUpdate = async (dept: Department) => {
        if (editingDeptId !== dept._id) return; // not in edit mode

        const trimmedName = editedName.trim();
        setEditingDeptId(null); // exit edit mode

        if (!trimmedName || trimmedName === dept.department) return; // no change
                ...
                <div className="flex flex-col gap-4 p-4">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <AddDepartmentModal onDepartmentsAdded={fetchDepartments} />
                            <Button
                    ...
                    <div className="flex items-center gap-2">
                        <Label htmlFor="page-size">Departments per page:</Label>
                        <select
                            id="page-size"
                            value={pageSize}
                            ...
                            departments.map((dept, index) => (
                                <TableRow key={dept._id}>
                                    ...
                                    <TableCell className="px-5 py-1 text-start text-theme-sm text-gray-600 dark:text-gray-400 flex items-center gap-2">
                                        <input
                                            ref={editingDeptId === dept._id ? inputRef : null} // 🔑 focus when editing
                                            type="text"
                                            value={editingDeptId === dept._id ? editedName : dept.department}
                                            readOnly={editingDeptId !== dept._id || !dept.isActive}
                                            ...
                                            onBlur={() => dept.isActive && handleNameUpdate(dept)}
                                            onKeyDown={(e) => {
                                                if (dept.isActive && e.key === "Enter") {
                                                    e.preventDefault();
                                                    handleNameUpdate(dept);
                                                }
                                            }}
                                            className={`w-full bg-transparent rounded px-1 py-0.5 focus:ring-2 ${dept.isActive
