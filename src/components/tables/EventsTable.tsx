...
export default function EventsTable({ initialData }: Props) {
    ...
    const fetchEvents = useCallback(async (abortController?: AbortController) => {
        setLoading(true);
        try {
            ...
            const response = await fetch(`/api/v1/admin/events/list?${params}`, {
                credentials: 'include',
            ...
            toast.error('Error fetching event list');
            setEvents([]);
            setTotalRecords(0);
            setTotalPages(1);
        } finally {
    ...
    // Fetch events when filters, page, or pageSize changes
    useEffect(() => {
    ...
    const handleEditClick = (event: Event) => {
        router.push(`/admin/events/add?id=${event._id}`);
    };

    const changeStatus = async (eventId: string, isActive: boolean) => {
        if (!eventId) return;

        const formData = new FormData();
        formData.append('isActive', (!isActive).toString());

        const promise = fetch(`/api/v1/admin/events/update/${eventId}`, {
            method: 'PATCH',
            credentials: 'include',
            body: formData,
        }).then(async (res) => {
        ...
        toast.promise(promise, {
            ...
            success: (res) => (res?.success ? 'Event updated successfully!' : null),
    ...
    const handleDownloadExcel = () => {
        ...
        const data = events.map((event, idx) => ({
            ...
            'Title': event.title,
            'Publish Date': event.publishDate || 'N/A',
            'Status': event.isActive ? 'Active' : 'Inactive',
            'CreatedAt': event.createdAt ? new Date(event.createdAt).toLocaleString() : "N/A",
        }));
            ...
            <PermissionGuard permission="event.read">
                <div className="flex flex-col gap-4 p-4">
                    ...
                    <div className="flex items-center gap-2">
                        <Label htmlFor="page-size">Events per page:</Label>
                        <select
                            id="page-size"
                            value={pageSize}
                ...
                <Table>
                    <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                        <TableRow>
                            ...
                            <TableCell isHeader className="px-5 py-3 font-medium text-start text-theme-xs text-gray-500">Publish Date</TableCell>
                        ...
                        {events.length > 0 ? (
                            events.map((event, index) => (
                                <TableRow key={event._id}>
                                    ...
                                    <TableCell className="px-5 py-1 text-start text-theme-sm text-gray-600 dark:text-gray-400">{event.title}</TableCell>
                                    <TableCell className="px-5 py-1 text-start text-theme-sm text-gray-600 dark:text-gray-400">{event.publishDate || 'N/A'}</TableCell>
                                    <TableCell className="px-5 py-1 text-start text-theme-sm text-gray-600 dark:text-gray-400">
                                        <label className="inline-flex items-center cursor-pointer">
                                            <input
                                                ...
                                                onChange={() => changeStatus(event._id, event.isActive)}
                                            />
