...
export default function DepartmentModal({ onDepartmentsAdded }: { onDepartmentsAdded: () => void }) {
    const [isOpen, setIsOpen] = useState(false);
    const [departments, setDepartments] = useState<string[]>([""]);

    // Add new empty input field
    const addField = () => setDepartments([...departments, ""]);
    ...
    // Submit multiple departments
    const handleSubmit = async () => {
        try {
            const filtered = departments.map((d) => d.trim()).filter(Boolean);
            if (filtered.length === 0) return;

            const res = await fetch("/api/v1/admin/departments/create", {
                method: "POST",
            ...
            onDepartmentsAdded?.(); // refresh list
            ...
            <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="relative z-50">
                ...
                <div className="fixed inset-0 flex items-center justify-center p-4">
                    <Dialog.Panel className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-md w-full">
                        <Dialog.Title className="text-lg font-semibold mb-4 dark:text-gray-300">Add Departments</Dialog.Title>
